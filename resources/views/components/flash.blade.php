...
@if (session()->has('message'))
    <div id="flash"
        class="fixed w-1/3 -top-20 -top-0 left-1/2 transform -translate-x-1/2 mt-5 px-10 py-3 bg-{{ $color_bg }} text-{{ $color_text }} shadow-lg rounded-lg z-40 transition-all">
        <p class="text-center">
            {{ session('message') }}
            @if (session()->has('link'))
