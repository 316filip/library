...
class Card extends Component
{
    ...
    public function __construct($data, $info)
    {
        ...
        $this->type = $type;
        $this->values = $data;
        $this->classes = (($number > 3 && ($type == 'book' || $type == 'work' || $type == 'booking') || $number > 5) ? 'hidden' : 'block') . ($number > 5 ? ' min-[470px]:hidden' : ' min-[470px]:block') . ($number > 5 ? ' md:hidden' : ' md:block') . ($number > 7 ? ' lg:hidden' : ' lg:block') . ($number > 4 ? ' xl:hidden' : ' xl:block') . ($number > 5 ? ' 2xl:hidden' : ' 2xl:block');
        $this->overlay = (($number != 3 && ($type == 'book' || $type == 'work' || $type == 'booking') || $number != 5) ? ' hidden' : ' block') . ($number != 5 ? ' min-[470px]:hidden' : ' min-[470px]:block') . ($number != 5 ? ' md:hidden' : ' md:block') . ($number != 7 ? ' lg:hidden' : ' lg:block') . ($number != 4 ? ' xl:hidden' : ' xl:block') . ($number != 5 ? ' 2xl:hidden' : ' 2xl:block');
        $this->filter = $filter;
